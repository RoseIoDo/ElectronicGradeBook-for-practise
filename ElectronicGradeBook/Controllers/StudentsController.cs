    ...
    public class StudentsController : Controller
    {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                return Json(new { success = true, message = "Студента видалено." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        [HttpGet]
        public async Task<IActionResult> GetFiltered([FromQuery] StudentFilterModel filter)
        {
            try
            {
                var pagedResult = await _service.GetFilteredAsync(filter);
